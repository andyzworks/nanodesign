...
failed=0
# Each run changes one variable relative to a recorded Stage 2 reference.
run_one 2 binder-seqonly-d32 --diffusion-batch-size 32 & p2=$!
run_one 3 binder-reference-no-ema --coordinate-loss-weight 4 --ema-decay 0 & p3=$!
for pid in "$p2" "$p3"; do
  wait "$pid" || failed=1
done
((failed == 0)) || exit 1
