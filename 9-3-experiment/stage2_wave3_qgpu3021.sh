...
failed=0
# Loss-scale is the only change from the matching Wave 2 sequence-only parent.
run_one 0 binder-seqonly-w1 & p0=$!
run_one 3 binder-seqonly-t05-w1 --training-noise-level 0.5 & p3=$!
for pid in "$p0" "$p3"; do
  wait "$pid" || failed=1
done
((failed == 0)) || exit 1
