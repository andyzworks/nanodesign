...
checkpoint="$repo/data/runs/nanodesign-v1/stage3-128/rna-reference-6000/milestones/samples-00024000.pt"
full_output="$repo/9-3-experiment/stage3_128_rna_24k_v2_audit.json"
median_output="$repo/9-3-experiment/stage3_128_rna_24k_median_noise_context_audit.json"
...
raise SystemExit(0 if any(r.get("global_samples_seen") == 24000 for r in records) else 1)
PY
  then
    break
  fi
...
if [[ ! -s "$full_output" ]]; then
  CUDA_VISIBLE_DEVICES=0 "$python" scripts/audit_stage2_checkpoint.py \
    ...
    --output "$full_output" \
    >9-3-experiment/stage3_128_rna_24k_v2_audit.log 2>&1 &
  full_pid=$!
else
  full_pid=""
fi
...
if [[ ! -s "$median_output" ]]; then
  CUDA_VISIBLE_DEVICES=1 "$python" scripts/audit_stage2_checkpoint.py \
    ...
    >9-3-experiment/stage3_128_rna_24k_median_noise_context_audit.log 2>&1 &
  median_pid=$!
else
  median_pid=""
fi
...
if (( full_status != 0 || median_status != 0 )); then
  echo "RNA 24K audit failed: full=$full_status median=$median_status" >&2
  exit 1
fi
