...
if [[ ! -s "$checkpoint_12k" ]]; then
  echo "missing required Stage-3 Binder 12K checkpoint: $checkpoint_12k" >&2
  exit 1
fi
...
"$python" scripts/audit_stage2_checkpoint.py \
  --checkpoint "$checkpoint_24k" \
  --task protein_binder \
  --weight-source ema \
  --generation-examples 8 \
  --protocol configs/evaluation/overfit128_v2.json \
  --output 9-3-experiment/stage3_128_protein_binder_24k_v2_audit.json
